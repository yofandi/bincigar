...
<div class="col-lg-12">
  <div class="card">
    <div class="card-body">
      <strong><i class="fa fa-home"></i> / Dashboard / Proses Produksi / Proses Cool Storage</strong>
    </div>
  </div>
</div>
<?php 
...
<div class="col-lg-12">
  <div class="card">
    <div class="card-header bg-primary text-light">
      <strong><i class="fa fa-gear"></i> Proses Cool  Storage| Hari Ini : <?php echo date('Y-m-d') ?></strong>
        ...
        <tbody>
          <?php $no=1;foreach ($Cool as $key): ?>
            <tr>
              ...
              <td>
                <a class="btn btn-warning btn-sm" href="<?php echo base_url('Proses_produksi/edit_cool/'.$key['id']) ?>"><i class="fa fa-edit"></i></a>
                 <a class="btn btn-danger btn-sm" href="<?php echo base_url('Proses_produksi/del_cool/'.$key['id']) ?>"><i class="fa fa-trash"></i></a>
              </td>
            </tr>
          <?php $no++;endforeach ?>
        </tbody>
    ...
    <div class="card-footer">
      <a href="<?php echo base_url("Proses_produksi/print_cool") ?>" class="btn btn-primary"><i class="fa fa-print"></i> Print</a>
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <label><i class="fa fa-gear"></i> Proses Cool</label>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title"></h4>
      </div>
      <div class="modal-body">
        <form method="post" action="<?php echo base_url('Proses_produksi/add_cool') ?>">
        <div class="col-lg-12">
        ...
        <div class="col-lg-5">
          ...
          <select name="jenis" id="jenis" class="form-control">
            <?php $db = $this->db->select('jenis')->get('jenis')->result(); 
            foreach ($db as $key): ?>
        ...
        <div class="col-lg-5">
          ...
          <select name="produk" id="merek" class="form-control">
            <?php $bd = $this->db->select('produk')->get('produk')->result(); 
            foreach ($bd as $lue): ?>
              <option value="<?= $lue->produk ?>"><?= $lue->produk ?></option>
            <?php endforeach ?>
          </select>
        </div>
        <div class="col-lg-2">
...
<div class="col-lg-12">
  <div class="card">
    <div class="card-header bg-warning text-light">
      <b><i class="fa fa-book"></i> Semua Laporan Cool Storage</b>
      <button type="button" class="btn btn-danger btn-sm pull-right" data-toggle="modal" data-target="#myModal2"><i class="fa fa-search"></i> FILTER DATA</button>
    </div>
    <div class="card-body" id="print_isi">
      <img src="<?php echo base_url('assets/images/BIN.png') ?>" class="hidd" style="width: 70px">
      <center>LAPORAN PROSES PRODUKSI (COOL STORAGE)<br>
...
<script>
  ...
  function cari(argument) {
    ...
    $.ajax({
      url : '<?php echo base_url("Proses_produksi/cari_cool") ?>',
      method : 'POST',
      ...
      success : function(data){
        ...
        $.ajax({
          url : '<?php echo base_url("Proses_produksi/total_cool") ?>',
          method : 'POST',
          data : {awal:awal, akhir:akhir, produk:produk},
          dataType : 'JSON',
  ...
  $('#ketahui').on('click', function(event) {
    ...
    $.ajax({
      url: '<?php echo base_url('Proses_produksi/drw_cl'); ?>',
      type: 'POST',
      dataType: 'JSON',
      data: {jenis:jenis, merek:merek},
      success : function(melo) {
        $('#cerutu').val(melo.dud);
      }
    })
  });
