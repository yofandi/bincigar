...
<img src="<?php echo base_url('assets/images/BIN.png') ?>" style="width: 70px">
	<center>
        ...
        <tbody>
          <?php $data=$this->db->order_by('id','desc')->get('wrapping')->result_array(); ?>
        ...
        <tfoot>
          <tr>
            ...
            <th>
              <?php echo $this->db->select('SUM(stock) as stock')->get('wrapping')->row()->stock; ?>
            </th>
            <th>
              <?php echo $this->db->select('SUM(terpakai) as terpakai')->get('wrapping')->row()->terpakai; ?>
            </th>
            <th>
              <?php echo $this->db->select('SUM(sisa) as sisa')->get('wrapping')->row()->sisa; ?>
            </th>
            <th>
              <?php echo $this->db->select('SUM(hasil) as hasil')->get('wrapping')->row()->hasil; ?>
            </th>
            <th>
              <?php echo $this->db->select('SUM(tambah_cerutu) as tambah_cerutu')->get('wrapping')->row()->tambah_cerutu; ?>
            </th>
            <th>
              <?php echo $this->db->select('SUM(hasil_akhir) as hasil_akhir')->get('wrapping')->row()->hasil_akhir; ?>
            </th>
            <th></th>
          </tr>
        </tfoot>
