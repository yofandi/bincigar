...
<div class="col-lg-12">
  <div class="card">
    <div class="card-body">
      <strong><i class="fa fa-home"></i> / Dashboard / Proses Produksi / Proses Wrapping</strong>
    </div>
  </div>
</div>
<?php 
...
<div class="col-lg-12">
  <div class="card">
    <div class="card-header bg-danger text-light">
      <strong><i class="fa fa-gear"></i> Proses Wrapping | Hari Ini : <?php echo date('Y-m-d') ?></strong>
        ...
        <tbody>
          <?php $no=1;foreach ($wrap as $key): ?>
            <tr>
              ...
              <td>
                <a class="btn btn-warning btn-sm" href="<?php echo base_url('Proses_produksi/edit_wrapping/'.$key['id']) ?>"><i class="fa fa-edit"></i></a>
                 <a class="btn btn-danger btn-sm" href="<?php echo base_url('Proses_produksi/del_wrapping/'.$key['id']) ?>"><i class="fa fa-trash"></i></a>
              </td>
            </tr>
          <?php $no++;endforeach ?>
        </tbody>
    ...
    <div class="card-footer">
      <a href="<?php echo base_url("Proses_produksi/print_wrapping") ?>" class="btn btn-primary"><i class="fa fa-print"></i> Print</a>
    </div>
  </div>
</div>
...
<div id="myModal" class="modal fade" role="dialog">
  <div class="modal-dialog modal-lg">
    <!-- Modal content--><?php $wrapping = $this->db->order_by('id','DESC')->get_where('data_stock', array('kategori'=>'Dekblad'))->row_array();?>
    <div class="modal-content">
      <div class="modal-header">
        <label><i class="fa fa-gear"></i> Proses Wrapping</label>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title"></h4>
      </div>
      <div class="modal-body">
        <form method="post" action="<?php echo base_url('Proses_produksi/add_wrapping') ?>">
        <div class="col-lg-12">
        ...
        <div id="hide">
          <div class="col-lg-6">
            <div class="form-group">
              <label>Stock Bakau (Dekblad):</label>
...
<div class="col-lg-12">
  <div class="card">
    <div class="card-header bg-danger text-light">
      <b><i class="fa fa-book"></i> Semua Laporan WRAPPING</b>
    ...
    <div class="card-body" id="print_isi">
      ...
      <center>LAPORAN PROSES PRODUKSI (WRAPPING)<br>
      ...
      <table class="table table-hover table-bordered">
        ...
        <tfoot class="bg-success text-light">
          <tr>
              <td></td><td>JUMLAH</td><td></td><td></td>
...
<script>
  ...
  function cari(argument) {
    ...
    $.ajax({
      url : '<?php echo base_url("Proses_produksi/cari_wrapping") ?>',
      method : 'POST',
      ...
      success : function(data){
        ...
        $.ajax({
          url : '<?php echo base_url("Proses_produksi/total_wrapping") ?>',
          method : 'POST',
          data : {awal:awal, akhir:akhir, produk:produk},
          dataType : 'JSON',
  ...
  $('#ketahui').on('click', function (event) {
    ...
    $.ajax({
      url: '<?php echo base_url('Proses_produksi/drw_wr'); ?>',
      type: 'POST',
      dataType: 'JSON',
      data: {jenis: jenis, merek: merek},
      success : function (data) {
        $('#hide').fadeIn();
        $('#kok').val(data.produksi_pr);
        $('#cerutu').val(data.hasil_akhir);
      }
    })
  });
</script>